...
- A user-requested metadata-only reissue of a deleted immutable GitHub Release follows the [reissue procedure](.agents/rules/release.md#reissue-a-deleted-immutable-release). It needs a reviewed reissue record and notes, points the new tag to the original release commit, and does not change the plugin's declared version.
