...
`scenarios.json` contains synthetic existing-project requests, each case's contract intent, realistic read-only project and package facts, and observable expectations for Motion Library adaptation. It covers the English plain-text handoff, Chinese labels and optional backticks, semantic automatic placement, an explicit time, ambiguous context, hardcoded source derivation, missing facts, successful and failed split cutout spans, an exact-version mismatch, a legacy `@aip` line whose "here" target remains explicit, and an ordinary project request that must not trigger the handoff. It contains no service credentials, real projects, transcripts or user media.
