...
Call `build_captions` with `project_id`, `caption_style` (the pattern), and any of `position`, `placement`, `hide_intervals`, `emphasis_word_ids`, `font`. It returns a task; read it with `get_task` (its `poll_args` say when). The finished task's `result` names the two files the service staged (`staged`), the pattern and position it baked, the emphasis it used as `word_id` values, and `host_div`, the one element for `index.html`:
