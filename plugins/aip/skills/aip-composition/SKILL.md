...
- Captions: a host div the same way, `data-composition-id="narrator-captions"` pointing at `compositions/narrator_captions.html`. That id is how the editor and the export find the caption layer: the editor reads the `.caption-band` element inside it, and both read the words from one inline `const WORDS = [...];` JSON array. The service's caption build is the only writer of those words: it writes that file and `compositions/transcript-src.json` from the transcript and returns the host div, the service rebuilds the words whenever the project is opened or edited, and a commit carrying caption words the build did not produce is refused. Caption text other than the transcript, such as a translation, is not supported yet; the [dynamic caption skill](../aip-dynamic-caption/SKILL.md) owns the style pick when it is listed. Mount that host div as returned. Its `.caption-band` is a full-canvas container, and the phrase inside it sits at the position the build reports, already clamped to the pattern's safe band; an offset on the band or the host from `index.html` (`top`, `inset`, `transform`, or any override) moves the whole canvas and puts every word off screen. To move the captions, call the build again with a different `position`. An edit without captions needs none of these files.
