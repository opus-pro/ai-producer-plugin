...
For a fresh project in Codex, use the [progressive checkpoint publisher](references/progressive-publication.md). Establish the complete planned speaker and audio timeline first. Then give every effect its own host execution checkpoint: author and mount only the current effect, invoke the checkpoint publisher at the end of that same shell call, and wait for its accepted result before generating any source for the next effect. Do not put future effect HTML, CSS, SVG, JavaScript, data, prompts, or per-effect branches in a generator before the current checkpoint returns. Do not run a loop that authors several effects before control returns to the model. Every intermediate index retains the full planned duration and speaker/audio ranges; unfinished beats play the presenter normally. Add framing transitions only for effects already mounted. Do not shorten the movie to the completed effects or group several effects into one progress step. The last effect closes authoring status.

The publisher performs signing, upload, commit and held completion checks without model turns. Appending it to the current effect's authoring command avoids a second model decision merely to publish; an edit with N effects adds at most N-1 model continuations for the visible authoring checkpoints. It uses Codex's local MCP transport and never approves requests or reads credentials. If that transport is unavailable, report the limitation; do not replace it with unbounded model-driven retries. In other hosts, or when editing an existing effect graph, use the ordinary complete-graph publication path and do not claim per-effect progress. A zero-effect edit also uses one ordinary final publication. Mechanical publication adds server work; measure total model-token cost when changing this flow.

The progressive publisher runs the local contract and timeline checks, signs the exact batch, uploads it, and waits for that commit's accepted receipt before returning. Do not duplicate those calls in the host. For ordinary publication, run [preflight.py](scripts/preflight.py) as described in the workspace reference, or skip it and let admission report contract errors if no Python 3.9+ interpreter is available; batch signatures and uploads with [upload_batch.py](scripts/upload_batch.py) or the host's batch HTTP tools, then commit with the current `base_digest`. Every commit remains all-or-nothing. A publisher failure stops its session without automatically retrying a mutation or adopting another writer's digest. Report it with the last successful effect count; do not restart the full sequence. Tool descriptions own authentication, limits, and task state.
