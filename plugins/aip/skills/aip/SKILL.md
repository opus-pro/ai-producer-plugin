...
Use two successful normal publications only when the host can run sign, upload, commit, and the commit's held completion check at an existing authoring boundary without splitting a model turn. The first must be a useful, independently valid partial cut. The final index must mount every requested moment, including the first cut and the remaining graph. Otherwise, author the complete graph and use one final publication. Do not fabricate progress with placeholder references or extra tool turns. This avoids mandatory new paid-provider or workflow-model calls, but token impact remains unmeasured until a paired run establishes it.

Before each real publication, run [preflight.py](scripts/preflight.py) as described in the workspace reference, or skip it and let admission report contract errors if no Python 3.9+ interpreter is available. Request signatures for that publication in a batch, run the uploads together with [upload_batch.py](scripts/upload_batch.py) or the host's batch HTTP tools, then commit with the current `base_digest`. Every commit remains all-or-nothing. If admission refuses deterministic contract errors, fix only that publication and resubmit it once. Report a second refusal and stop; do not expand this into playback or aesthetic repair. Tool descriptions own authentication, limits, and task state.
