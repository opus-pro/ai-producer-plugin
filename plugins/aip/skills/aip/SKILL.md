...
For a fresh project in Codex, use the [progressive publisher](references/progressive-publication.md) inside the authoring script. Establish the complete planned speaker and audio timeline first. Write one completed effect and its dependencies, mount it, and publish it before writing the next effect. Every intermediate index retains the full planned duration and speaker/audio ranges; unfinished beats play the presenter normally. Add framing transitions only for effects already mounted. Do not shorten the movie to the completed effects, build the entire effect set before publishing, or group several effects into one progress step. The last effect closes authoring status.

The publisher performs signing, upload, commit and held completion checks without model turns. Keep those calls inside the same authoring execution, not separate model decisions for each effect. It uses the existing Codex MCP connection and never approves requests or reads credentials. If the native transport is unavailable, report that limitation; do not replace it with extra model-driven publication rounds. In other hosts, or when editing an existing effect graph, use the ordinary complete-graph publication path and do not claim per-effect progress. A zero-effect edit also uses one ordinary final publication. Mechanical publication adds server work; overall model-token cost still requires measurement.
