...
For a fresh project in Codex, use the [progressive checkpoint flow](references/progressive-publication.md). Establish the complete planned speaker and audio timeline first. Then give every effect its own accepted checkpoint: author and mount only the current effect, prepare its exact batch locally, and use the AIP MCP tools already loaded in the current task to sign, upload, commit, and wait for acceptance. Record the accepted receipt locally before generating any source for the next effect. Do not launch another Codex CLI, app server, task, or model subtask for publication. Do not put future effect HTML, CSS, SVG, JavaScript, data, prompts, or per-effect branches in a generator before the current receipt is recorded. Do not run a loop that authors several effects before control returns to the model. Every intermediate index retains the full planned duration and speaker/audio ranges; unfinished beats play the presenter normally. Add framing transitions only for effects already mounted. Do not shorten the movie to the completed effects or group several effects into one progress step. The last effect closes authoring status.

The local helper validates timeline state and receipts without authentication, credentials, network calls, or model invocation. Publication uses the current task's normal plugin connection, so each effect adds bounded MCP result continuations for signing, committing, and the held task wait, but it creates no additional Codex task and no provider call. In other hosts, or when editing an existing effect graph, use the ordinary complete-graph publication path and do not claim per-effect progress. A zero-effect edit also uses one ordinary final publication. Mechanical publication adds server work and the current host must read its tool results; measure total model-token cost when changing this flow.
...
After the final requested graph is accepted, deliver the durable project link (`page_url`, retrieved if missing) in the final reply, then stop unless an export was explicitly requested. Translate refusals and material warnings under the user-facing language contract; do not repeat internal codes or mechanisms. Do not claim visual quality, audio synchronization, or export compatibility. If the finished preview was not watched, say that plainly and ask the user to review it at the link.
