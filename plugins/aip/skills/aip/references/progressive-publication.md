...
The caption layer is not a batch step: its build needs the committed root, and the service stages its files, so the helper cannot hash them. Await the last batch's execution to its `accepted: true` result, then call the caption build (the [dynamic caption skill](../../aip-dynamic-caption/SKILL.md) owns the pick when it is listed; the [composition contract](../../aip-composition/SKILL.md) owns the host div) and publish it with one ordinary commit: read `list_workspace` once for the current digest, mount the returned `host_div` in `index.html` as written, run preflight declaring the result's paths as `--remote-file` (the declared caption composition the host cannot read is reported as `composition_not_locally_inspectable` and is a warning), sign and upload `index.html`, and commit with that `base_digest`, `expected_files` naming `index.html` and the result's `files` entries exactly as returned, and `authoring: false`. That commit is the one that closes authoring. The caption files stay staged until it; a commit that leaves them out reports them in `retained_staged`, and a caption build that fails leaves authoring open for the retry or for a caption-free closing commit.
