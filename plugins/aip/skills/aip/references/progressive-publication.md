...
The helper prepares and publishes each step automatically: wait for the preceding accepted checkpoint, validate and install its isolated draft, commit the frozen files, and accept the exact task's successful terminal receipt. Markup, stylesheets and JSON ride on the commit itself, so an ordinary effect signs nothing and uploads nothing; only bytes that commit cannot carry - an image, a font, a video, an audio file, or a batch past the service's inline budget - still take the signed upload the helper runs for them. Its held waits stop after ten minutes or forty replies. Only one signing/upload/commit sequence is in flight. The first step can join across both effects of the preceding batch; subsequent steps use its accepted digest directly. Only the last step of a final batch closes authoring. Tool descriptions own service limits and refusals; a batch does not bypass commit limits.
...
The caption layer is not a batch step: its build needs the committed root, and the service stages its files, so the helper cannot hash them. Await the last batch's execution to its `accepted: true` result, then call the caption build (the [dynamic caption skill](../../aip-dynamic-caption/SKILL.md) owns the pick when it is listed; the [composition contract](../../aip-composition/SKILL.md) owns the host div) and publish it with one ordinary commit: read `list_workspace` once for the current digest, mount the returned `host_div` in `index.html` as written, run preflight declaring the result's paths as `--remote-file` (the declared caption composition the host cannot read is reported as `composition_not_locally_inspectable` and is a warning), and commit with that `base_digest`, `expected_files` carrying `index.html` as a `content` entry and the result's `files` entries exactly as returned, and `authoring: false`. That `content` is the file's bytes as written, read from disk in the same call that commits; never retype or re-emit the document from memory, because the service hashes what it is handed, so a clause dropped on the way out is published as if it were authored. If the service refuses the entry for its size, upload that one file with `sign_workspace_upload` and name it here by `sha256` instead. That commit is the one that closes authoring. The caption files stay staged until it; a commit that leaves them out reports them in `retained_staged`, and a caption build that fails leaves authoring open for the retry or for a caption-free closing commit.
