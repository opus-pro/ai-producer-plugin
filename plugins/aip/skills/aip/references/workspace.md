...
Write every HTML `src`, `href`, `data-composition-src`, and `data-pip-src`, and every CSS `url(...)` inside an HTML document, from the render-engine root, whichever directory holds the document: the editor mounts a composition's template into `index.html`, so a document in `compositions/` references an image as `public/images/example.jpg` exactly as `index.html` does. A `../public/...` path from a composition names a file above the served root; the editor cannot load it and `commit_workspace` refuses it as `missing_reference`. A `url(...)` inside a `.css` file under `styles/` resolves from that file, as CSS does.
