...
6. **Caption windows** (optional). `caption_windows` is a list of `{start, end, position?, plate?}` on the cut, one per visual moment whose band differs from the whole-video anchor. `position` is that window's anchor as a percent of frame height; the [framing skill](../aip-framing/SKILL.md) gives the value per layout and canvas (`Where the caption sits`). `plate: "dark"` backs each caption phrase in that window with one near-black plate as long as the phrase, for a moment whose composited ground under the band is light; the default black canvas needs none. Each window names at least one of the two. Write the values on that moment's host in `index.html` (`data-hide-captions="false"`, `data-caption-position`, `data-caption-plate`) from the task result's `windows`, which is each window as the service baked it (an anchor may have been clamped into the pattern's band), because the editor and the export re-derive the caption layer from those attributes after any edit; the call bakes the first copy, and the composition clamps any later value into the same band. A top-level `plate: "dark"` backs the whole video and is for a brief that asks for boxed captions. The task result's `windows` lists each window as baked (an anchor clamped into the pattern's band; a plate dropped on `inverted-stack`, which blends the footage), and `band` is the pixel box the whole-video anchor produces (`anchor`, `top_px`, `bottom_px`), the region a seat or a graphic keeps clear.
